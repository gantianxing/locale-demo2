...
<@spring.message "select.language"/> : <span><a href="javascript:void(0)" onclick="changeLanguage('en_GB')">EN</a></span>
    ...
    <form name="user" action="save" method="post">
        <fieldset>
            ...
            <p>
            <@spring.bind "user.phoneNum" />
                <label for="phoneNum"><@spring.message "label.phoneNum"/>: </label>
                <input id="phoneNum" type="text" name="phoneNum" value="${user.phoneNum}" cssErrorClass="error"/>
            <@spring.showErrors "<br/>" cssClass="error"/>
            </p>
            <p>
            <@spring.bind "user.regDate" />
                <label for="regDate"><@spring.message "label.regDate"/>: </label>
                <input id="regDate" type="text" name="regDate" value="<#if (user.regDate)??> ${(user.regDate?date)} </#if>" cssErrorClass="error"/>
            <@spring.showErrors "<br/>" cssClass="error"/>
            </p>
            <p id="buttons">
